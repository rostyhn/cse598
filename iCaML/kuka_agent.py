...
class KukaAgent():
    def __init__(self, env, motors, ground_actions=False):
        ...
        if not os.path.exists(files_dir):
            os.mkdir(files_dir)
        self.translator = KukaTranslator(env, motors, ground_actions= ground_actions,files_dir=files_dir)
        ...
        try :
            ...
            temp_states = self.generate_random_states(n = num_random_states, algo = "human",abstract = True,random = True, save_trace = True)
    ...
    def generate_random_states(self,n=5,save=True,abstract = False,random = False,save_trace = False,add_intermediate=True,algo = "custom_astar"):
        ...
        new_traces = []
        initial_random_states = [self.translator.generate_random_state() for i in range(n)]
        if add_intermediate:
            abs_random_states = []
