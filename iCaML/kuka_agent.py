...
class KukaAgent:
    ...
    def show_actions(self, action=None):
        ...
        for k, v in self.actions.items():
            ...
            print("before")
            self.environment["gym_env"].restore_state(s1.pyBulletStateID)
            time.sleep(3)
            print("after")
            self.environment["gym_env"].restore_state(s2.pyBulletStateID)
            time.sleep(3)
    ...
    def solve_game(self, state):
        ...
        while not done:
            ...
            states.append(KukaState(self.environment))
