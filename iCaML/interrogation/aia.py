...
class AgentInterrogation:
    ...
    def __init__(
        ...
        # try:
        #     if load_old_q:
        #         with open(self.save_q_file,"rb") as f:
    ...
    # def save_q(self):
    #     '''Saves the query dict to file for future use. '''
    #     with open(self.save_q_file,'wb') as f:
    ...
    def get_next_pal_tuple(self, action="", predicate="", location=0):
        ...
        #             (location == 0) and key not in self.difficult_pal_tuple:
        #         return key
        #     if not val and (action == "" or action == key[0]) and (predicate == "" or predicate == key[1]) and \
            ...
            while not found_state and init_state_tried < 1000:
                for plan in valid_plans:
                    ...
                    for _state in temp_new_init_states:
                        ...
                        #     pre_state = self.agent.agent_model.translator.high_actions['a11'][0]
                        #     test_state = self.agent.agent_model.translator.get_ground_state(pre_state)
                        #     _state = test_state
                        # if plan == ['a59']:
    ...
    def agent_interrogation_algo(self):
        ...
        while True:
            # with open("../gvg_agents/files/"+self.domain_name+"/checkpoint","wb") as f:
            #     temp = {}
            #     temp['valid_models'] = valid_models
            #     temp['pal_tuple_dict'] = self.pal_tuple_dict
            #     temp['next_pal_tuple'] = next_pal_tuple
            #     temp['abs_predicates'] = abs_predicates
            #     pickle.dump(temp,f)

            model_level = 0
            previous_pal_tuple = copy.deepcopy(next_pal_tuple)
